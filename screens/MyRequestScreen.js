...
//this screen will only show the requests of the current users.
export default class MyRequestScreen extends Component{
  ...
  getItems =()=>{
    this.requestRef = db.collection("Exchange_Request").where('user_id',"==",this.state.barterId).onSnapshot((snapshot)=>{
    ...
    return(
      <View style={styles.box}>
        <AppHeader
          ...
          title="My Requests" navigation ={this.props.navigation} 
        ...
        <View>
          {
            ...
            ?(
              <View>
                <Text style={styles.modalTitle}>You have not added any Request or Item</Text>
              </View>
            )
            :(
              <FlatList
